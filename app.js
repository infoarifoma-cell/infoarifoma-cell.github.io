...
const MACHINES=[
  ...
  {id:'M1',name:'Machacadora RT12010',tipo:'MACHACADORA',modelo:'RT12010',fabricante:'MOPSA'},
...
const GAMAS=[
  ...
  {id:'MACH-80H',modelo:'RT12010',nombre:'80 Horas',intervalo:80,checks:['Engrase rodamientos (80 Gramos c/u) NLGI2.','Revisión de los asientos de las mandíbulas.','Revisión aprietes de los tacos de goma.','Revisión del reglaje entre pico y valle.','Engrase del cardán del alimentador.','Verificar tensión de las correas.','Revisión resortes laterales y principales.','Revisión del nivel de aceite (ISO200).','Revisión tope de seguridad.']},
...
async function goStep2(){if(!selMachine)return;const grid=document.getElementById('gamaGrid');grid.innerHTML='<div class="no-results">Cargando gamas...</div>';showOT('screen2');setSteps(2);
  ...
  try{
    ...
    console.log('DEBUG gamas: buscando modelo=['+selMachine.modelo+'], normasData count='+normasData.length+', modelos en BD='+JSON.stringify([...new Set(normasData.map(n=>n.Modelo))]));
    const dynGamas=normasData.filter(n=>(n.Modelo||'').trim().toUpperCase()===(selMachine.modelo||'').trim().toUpperCase());
    console.log('DEBUG gamas: encontradas '+dynGamas.length+' para '+selMachine.modelo);
    dynGamas.forEach(n=>{
