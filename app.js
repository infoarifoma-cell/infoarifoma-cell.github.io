...
function goStep4Real(){
  // Acumular checks de gamas anteriores (intervalo menor, mismo modelo)
  const mod = (selMachine.modelo||'').trim().toUpperCase();
  const gamasModelo = normasData.filter(n=>(n.Modelo||'').trim().toUpperCase()===mod);
  const checksAcumulados = [];
  // Ordenar por intervalo y añadir checks de gamas con intervalo < selGama.intervalo
  gamasModelo
    .filter(n=>(n.Intervalo||0) < selGama.intervalo && (n.Intervalo||0) > 0)
    .sort((a,b)=>(a.Intervalo||0)-(b.Intervalo||0))
    .forEach(n=>{
      const checks=[];for(let i=1;i<=60;i++)if(n['n'+i])checks.push(n['n'+i]);
      if(checks.length) checksAcumulados.push({nombre:n.Gama||n.Numero||'Gama '+n.Intervalo+'h', checks});
    });

  // Construir lista completa: checks acumulados + checks de la gama actual
  const todosChecks = [];
  checksAcumulados.forEach(g=>g.checks.forEach(c=>todosChecks.push({text:c,grupo:g.nombre})));
  selGama.checks.forEach(c=>todosChecks.push({text:c,grupo:selGama.nombre}));

  checkStates=new Array(todosChecks.length).fill(false);
  document.getElementById('checkTitle').textContent=selMachine.name;
  document.getElementById('checkBanner').textContent=`⏱ Gama: ${selGama.nombre}  ·  ${todosChecks.length} puntos`;
  const list=document.getElementById('checkItems');list.innerHTML='';
  let grupoActual = null;
  todosChecks.forEach((c,i)=>{
    if(c.grupo !== grupoActual) {
      grupoActual = c.grupo;
      const sep=document.createElement('div');
      sep.style.cssText='font-size:.65rem;font-weight:700;color:var(--muted);padding:8px 0 2px 8px;text-transform:uppercase;letter-spacing:.05em';
      sep.textContent=c.grupo;
      list.appendChild(sep);
    }
    const item=document.createElement('div');item.className='check-item';
    item.innerHTML=`<div class="check-box"></div><div class="check-text">${c.text}</div>`;
...
function goStep5(){const tipos={preventivo:'Preventivo programado',correctivo:'Correctivo',revision:'Revisión rápida'};const tipo=document.getElementById('inputTipo').value;const horas=document.getElementById('inputHoras').value;const obs=document.getElementById('inputObs').value;const operario=document.getElementById('inputOperario').value||'—';const estadoOT=document.getElementById('inputEstado').value==='cerrada'?'Cerrada':'Abierta';const fechaRaw=document.getElementById('inputFecha').value;const fecha=fechaRaw?new Date(fechaRaw).toLocaleString('es-ES',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}):'—';document.getElementById('summaryMain').innerHTML=`<div class="sum-box"><div class="sum-row"><span class="sum-key">Máquina</span><span class="sum-val">${selMachine.name}</span></div><div class="sum-row"><span class="sum-key">Código</span><span class="sum-val">${selMachine.id}</span></div><div class="sum-row"><span class="sum-key">Fecha</span><span class="sum-val">${fecha}</span></div><div class="sum-row"><span class="sum-key">Operario</span><span class="sum-val">${operario}</span></div><div class="sum-row"><span class="sum-key">Estado</span><span class="sum-val">${estadoOT}</span></div><div class="sum-row"><span class="sum-key">Horómetro</span><span class="sum-val">${horas} h</span></div><div class="sum-row"><span class="sum-key">Gama</span><span class="sum-val">${selGama.nombre}</span></div><div class="sum-row"><span class="sum-key">Tipo</span><span class="sum-val">${tipos[tipo]}</span></div>${obs?`<div class="sum-row"><span class="sum-key">Obs.</span><span class="sum-val">${obs}</span></div>`:''}</div>`;const checksShow = selGama._checksExpanded || selGama.checks;
document.getElementById('summaryChecks').innerHTML=`<div class="sum-box">${checksShow.map((c,i)=>`<div class="sum-row"><span class="sum-key" style="max-width:70%;font-size:.62rem">${c}</span><span class="sum-val" style="color:${checkStates[i]?'var(--accent2)':'var(--muted)'}">${checkStates[i]?'✓ OK':'— Pendiente'}</span></div>`).join('')}</div>`;showOT('screen5');setSteps(5);}
async function submitOT(){
  ...
  // Guardar también los nombres de checks expandidos en texto (observaciones internas)
  if(selGama._checksExpanded) selGama._checksForSave = selGama._checksExpanded;
...
function printOTHistorial(id){
  ...
  // Fallback: OTs antiguos guardaban el nombre de la máquina como gama — buscar por activo
  if(!gama||!gama.checks||!gama.checks.length){
    const todasGamas=getEffectiveGamas().filter(g=>g.checks&&g.checks.length);
    // Buscar gama cuyo modelo coincida con alguna variante del id del activo
    const activo=r.activo||'';
    const checks0=Array.isArray(r.checks)?r.checks:[];
    const nChecksTrue=checks0.filter(Boolean).length;
    // Elegir la gama cuyo número de checks se acerque más al número de true en el OT
    const candidatas=todasGamas.filter(g=>{
      // Mismo activo hardcodeado o modelo coincide parcialmente
      const maq=MACHINES.find(m=>m.id===activo);
      return maq && (g.modelo===maq.modelo || maq.modelo.includes(g.modelo) || g.modelo.includes(maq.modelo));
    });
    if(candidatas.length===1) gama=candidatas[0];
    else if(candidatas.length>1) gama=candidatas.reduce(function(best,g){ return Math.abs(g.checks.length-nChecksTrue)<Math.abs(best.checks.length-nChecksTrue)?g:best; },candidatas[0]);
    // Último recurso: gama cuyo nº de checks coincide exactamente con el array del OT
    if(!gama||!gama.checks||!gama.checks.length){
      const exacta=todasGamas.find(g=>g.checks.length===checks0.length);
      if(exacta) gama=exacta;
    }
  }
...
function renderPrecioArido(){
  ...
  // Row: Precio socios global (+8%)
  html+='<tr style="background:rgba(46,125,107,.08);font-weight:700"><td>Precio Socios Global (+8%) (€/Tn)</td>';
  for(const m of mesesActivos){
    const g=gastoMes[m]||0; const tn=prodMes[m]?.tn||0;
    html+=`<td class="costes-val" style="color:#2e7d6b">${tn?fmtES(g/tn*1.08):'—'}</td>`;
  }
  html+=`<td class="costes-val costes-total-col" style="color:#2e7d6b">${totalProd?fmtES(totalGasto/totalProd*1.08):'—'}</td></tr>`;
  ...
  // Per product type — 0/4 absorbe 60% del coste, resto 40% (÷3)
  const COSTE_PCT = {t04:0.60, t412:0.40/3, t1220:0.40/3, t2040:0.40/3};

  for(const tipo of tipos){
    const pct = COSTE_PCT[tipo.key];
    ...
    for(const m of mesesActivos){
      const tn=prodMes[m]?.[tipo.key]||0; const g=gastoMes[m]||0;
      const precio=tn?(g*pct)/tn:0;
      html+=`<td class="costes-val">${precio?fmtES(precio):'—'}</td>`;
    }
    const precioTotal=tProd?(totalGasto*pct)/tProd:0;
    ...
    for(const m of mesesActivos){
      const tn=prodMes[m]?.[tipo.key]||0; const g=gastoMes[m]||0;
      const precio=tn?(g*pct)/tn*1.08:0;
...
let _fpSortCol = null;
let _fpSortDir = 1; // 1 asc, -1 desc
let _fpFiltro = '';

function fpSetTab(tab) {
  _fpTab = tab;
  _fpSortCol = null; _fpFiltro = '';
  const fi = document.getElementById('fp-filtro'); if(fi) fi.value='';
...
function _fpSort(col) {
  if (_fpSortCol === col) _fpSortDir *= -1;
  else { _fpSortCol = col; _fpSortDir = 1; }
  renderFacturasPendientes();
}

function _fpFiltrar() {
  _fpFiltro = (document.getElementById('fp-filtro').value || '').toLowerCase();
  renderFacturasPendientes();
}

function _fpTh(label, col) {
  const arrow = _fpSortCol===col ? (_fpSortDir===1?' ▲':' ▼') : '';
  return `<th onclick="_fpSort('${col}')" style="padding:6px 8px;border:1px solid var(--border);text-align:left;white-space:nowrap;cursor:pointer;user-select:none">${label}${arrow}</th>`;
}
...
function renderFacturasPendientes() {
  ...
  let data = esVenta ? [..._fpVentaData] : [..._fpCompraData];

  // Filtro texto
  if (_fpFiltro) {
    data = data.filter(r => {
      const hay = esVenta
        ? `${r.number} ${r.customerName} ${r.invoiceDate} ${r.dueDate}`
        : `${r.number} ${r.vendorName} ${r.vendorInvoiceNumber} ${r.invoiceDate} ${r.dueDate}`;
      return hay.toLowerCase().includes(_fpFiltro);
    });
  }
  ...
  let html = `<div style="margin-bottom:.6rem">
    <input id="fp-filtro" type="text" placeholder="Filtrar por nombre, nº..." value="${_fpFiltro}"
      oninput="_fpFiltrar()"
      style="padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:.78rem;width:260px;background:var(--surface);color:var(--text)">
  </div>`;

  if (!data.length) {
    wrap.innerHTML = html + '<div style="color:var(--muted);text-align:center;padding:30px;font-size:.82rem">Sin resultados.</div>';
    return;
  }

  if (esVenta) {
    const totalPend = data.reduce((a,r)=>a+(parseFloat(r.remainingAmount)||0),0);
    html += `<div style="font-size:.78rem;color:var(--accent2);font-weight:700;margin-bottom:.5rem">${data.length} facturas · Total pendiente: ${totalPend.toLocaleString('es-ES',{minimumFractionDigits:2})} €</div>`;
    html += '<table style="width:100%;border-collapse:collapse;font-size:.78rem"><thead><tr style="background:var(--surface2)">';
    html += _fpTh('Nº','number') + _fpTh('Fecha registro','invoiceDate') + _fpTh('Cliente','customerName') + _fpTh('Fecha vencimiento','dueDate') + _fpTh('Importe IVA incl.','totalAmountIncludingTax') + _fpTh('Importe pendiente','remainingAmount');
    html += '</tr></thead><tbody>';
    data.forEach((r,i) => {
      const bg = i%2===0 ? '' : 'background:var(--surface2)';
...
function exportarFacturasPendientesExcel() {
  ...
  if (esVenta) {
    ...
    const filas = data.map(r => [
      ...
      (parseFloat(r.remainingAmount)||0).toFixed(2).replace('.',',')
    ].join(sep));
    csv = bom + cab + '\n' + filas.join('\n');
  } else {
  ...
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `facturas-pendientes-${esVenta?'venta':'compra'}-${new Date().toISOString().slice(0,10)}.csv`;
  a.click();
  URL.revokeObjectURL(a.href);
}
...
async function _ensayosGenerarPDF() {
  ...
  for (const sem of sems) {
    ...
    // Fecha de recogida — se deja en blanco para que el laboratorio la rellene
    ...
    // ⚠️ ESPECÍFICO PARA PLANTILLA ESOCAN (PLANTILLA ENSAYOS.pdf)
    // Si se añade otro proveedor con plantilla diferente, crear nueva función con sus coordenadas
    const COORDS = {
      'granulometria|0/4':    {x:310, y:626},
      'granulometria|4/12':   {x:335, y:626},
      'granulometria|12/20':  {x:360, y:626},
      'granulometria|20/40':  {x:385, y:626},
      ...
      'ind_lajas|4/12':       {x:335, y:583},
      'ind_lajas|12/20':      {x:360, y:583},
      'ind_lajas|20/40':      {x:385, y:583},
...
function _ensayosRenderRegistros() {
  ...
  // Estilos cabecera
  const TH_GRN = 'padding:4px 6px;border:1px solid #7eb87e;background:#70ad47;color:#fff;text-align:center;font-size:.72rem;font-weight:700;white-space:nowrap;line-height:1.2;';
  const TH_GRN2 = 'padding:2px 3px;border:1px solid #7eb87e;background:#548235;color:#fff;text-align:center;font-size:.68rem;font-weight:700;white-space:nowrap;width:26px;min-width:22px;';
  const TH_PRP = 'padding:4px 6px;border:1px solid #9b8fbf;background:#7030a0;color:#fff;text-align:center;font-size:.72rem;font-weight:700;white-space:nowrap;line-height:1.2;';
  const TH_PRP2 = 'padding:2px 3px;border:1px solid #9b8fbf;background:#4b1875;color:#fff;text-align:center;font-size:.68rem;font-weight:700;white-space:nowrap;width:26px;min-width:22px;';
  const TH_HDR = 'padding:4px 6px;border:1px solid #ccc;background:#1e2a1e;color:#fff;text-align:center;font-size:.72rem;font-weight:700;white-space:nowrap;';
  const TD = 'padding:4px 7px;border:1px solid #dde;font-size:.78rem;text-align:center;';
  const TD_CNC = 'padding:2px 3px;border:1px solid #dde;font-size:.72rem;text-align:center;font-weight:700;width:26px;min-width:22px;';
  ...
  extraCols.forEach(function(c){
    var span = c.cnc ? 2 : 1;
    html += '<th colspan="' + span + '" rowspan="' + (c.cnc ? 1 : 2) + '" style="' + TH_PRP + 'vertical-align:bottom;padding:4px 2px"><div style="writing-mode:vertical-rl;transform:rotate(180deg);white-space:nowrap;font-size:.6rem;line-height:1.1;max-height:70px;overflow:hidden">' + c.label + '</div></th>';
  });
...
async function ensayosRecalcularCNC() {
  ...
  // Agrupar registros igual que en render
  var grupos = {};
  _ensayosRegistros.forEach(function(r) {
    var key = r.fraccion + '||' + (r.num_albaran || (r.semana_id + '_' + (r.fecha_toma||'')));
    if (!grupos[key]) grupos[key] = { fraccion: r.fraccion, regs: [] };
    grupos[key].regs.push(r);
  });

  var updates = [];
  Object.values(grupos).forEach(function(g) {
    // Combinar todos los resultados del grupo
    var combinado = {};
    g.regs.forEach(function(r) {
      var res = r.resultados || {};
      Object.keys(res).forEach(function(k) {
        if (k !== '_gran_cnc' && k !== '_cnc') combinado[k] = res[k];
      });
    });

    var auto = _calcularCNC(g.fraccion, combinado);
    ...
    g.regs.forEach(function(r) {
      ...
      // Quitar claves internas viejas
      delete res._gran_cnc; delete res._cnc;
      // Inyectar solo las que corresponden a este tipo de registro
      if (r.tipo_ensayo === 'granulometria' && Object.keys(auto._gran_cnc).length) {
        res._gran_cnc = auto._gran_cnc;
      }
      if (r.tipo_ensayo !== 'granulometria' && Object.keys(auto._cnc).length) {
        // Solo inyectar _cnc en el registro cuyo key esté en auto._cnc
        var misCnc = {};
        Object.keys(auto._cnc).forEach(function(k) {
          if (res[k] != null) misCnc[k] = auto._cnc[k];
        });
        if (Object.keys(misCnc).length) res._cnc = misCnc;
      }
